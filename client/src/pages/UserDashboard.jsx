...
return (
  <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-white to-blue-100 text-gray-800">
    <main className="max-w-7xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
      ...
      <h3 className="text-2xl font-semibold mt-12 mb-6 text-indigo-700">Features</h3>
      ...
      <section className="mt-14">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4">
          <h3 className="text-2xl font-semibold text-indigo-700">Recent Activities</h3>
          <select
            value={activityLimit}
      ...
      <section className="mt-14">
        <h3 className="text-2xl font-semibold mb-4 text-indigo-700">Your Files</h3>
        {loading ? (
          <div className="flex justify-center items-center p-6 text-indigo-600 font-medium">
            Loading...
          </div>
        ) : error ? (
          <div className="bg-red-500 text-white p-4 rounded shadow">{error}</div>
        ) : files.length === 0 ? (
              ...
              <tbody>
                {files.map((file) => (
                  <tr key={file._id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="p-4">{(file.size / 1024 / 1024).toFixed(2)} MB</td>
                    <td className="p-4">
                      {format(new Date(file.createdAt), "MMM d, yyyy h:mm a")}
                    </td>
                    <td className="p-4">
                      {file.lastAnalyzed
                        ? format(new Date(file.lastAnalyzed), "MMM d, yyyy h:mm a")
                        : "Never"}
                    </td>
